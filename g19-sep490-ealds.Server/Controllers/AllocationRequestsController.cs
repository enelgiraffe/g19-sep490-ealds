...
public class AllocationRequestsController : ControllerBase
{
    ...
    public AllocationRequestsController(IAllocationRequestService service)
    {
        _service = service;
    }
    ...
    public async Task<ActionResult<int>> GetWarehouseAvailable([FromQuery] int assetId)
        => await ExecuteAsync(() => _service.GetWarehouseAvailableAsync(assetId));
    ...
    public async Task<IActionResult> ListAllocationOrdersSummary()
    {
        if (!TryGetCurrentUserId(out var userId)) return Unauthorized();

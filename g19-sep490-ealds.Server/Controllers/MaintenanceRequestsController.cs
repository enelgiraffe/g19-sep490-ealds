...
public class MaintenanceRequestsController : ControllerBase
{
    ...
    public async Task<IActionResult> StartMaintenance(int id, [FromBody] MaintenanceStartDto dto)
    {
        ...
        var hasAllowedRoleFromClaims = User.Claims.Any(c =>
        {
            var isRoleClaim = c.Type == ClaimTypes.Role
                              || string.Equals(c.Type, "role", StringComparison.OrdinalIgnoreCase)
                              || string.Equals(c.Type, "roles", StringComparison.OrdinalIgnoreCase);
            if (!isRoleClaim || string.IsNullOrWhiteSpace(c.Value)) return false;
            var normalized = c.Value.Trim().ToUpperInvariant();
            return normalized is "DIRECTOR" or "DEPARTMENTMANAGER" or "DEPARTMENT_MANAGER" or "DEPT_MANAGER"
                or "DEPT_HEAD" or "DEPARTMENT_HEAD" or "HEAD_OF_DEPARTMENT"
                or "TRUONG_PHONG" or "TRUONGPHONG" or "ACCOUNTANT";
        });
        ...
        var allowedByRole = hasAllowedRoleFromToken || hasAllowedRoleFromClaims || userRoles.Any(ur =>
            ...
            (ur.Role?.Code != null &&
                (
                    ...
                    ur.Role.Code.ToUpper() == "DEPT_HEAD" ||
    ...
    public async Task<IActionResult> StartScheduledMaintenanceTask(int taskId, [FromBody] MaintenanceStartDto dto)
    {
        if (dto == null)
            return BadRequest("Request body is required.");
        if (dto.StartedBy <= 0)
        ...
        var task = await _db.MaintenanceTasks.FirstOrDefaultAsync(t => t.TaskId == taskId);
        if (task == null)
            return NotFound();
        if (task.AssetRequestId.HasValue)
            return BadRequest("Task is linked to a maintenance request. Use /api/Assets/Requests/maintenance/{assetRequestId}/start.");
        if (task.Status != 0)
            return BadRequest("Maintenance can only be started while the task is pending.");
        ...
        return Ok(new { taskId = task.TaskId, status = task.Status });
    }
